...
The AI-Assisted SDLC framework uses **7 stages** that balance comprehensive coverage with practical simplicity. The stages are designed to be:
...
- **Cross-functional** - Clear handoffs between business and technical stakeholders
- **AI-friendly** - Each stage has specific guidance for leveraging AI assistance
...
### Purpose
Establish the business case, define objectives with measurable success criteria, set scope boundaries, and produce a plan to reach the first major decision gate (proceed/pivot/stop).
...
### Purpose
Convert business objectives into testable requirements with acceptance criteria that enable coherent design decisions, accurate test planning, and reduced rework from ambiguity.
...
- Identify non-functional requirements (NFRs): performance, security, scalability, usability
...
### Outputs
- **Requirements Document** - Functional and non-functional requirements (see `requirements-brief.md`)
...
### Handoff to Next Stage
Design stage receives: Requirements document with acceptance criteria, prioritized backlog, NFRs, and success criteria to be instrumented.
...
**Execution Pattern:** Foundational + Iterative (initial architecture, then per-increment designs)

### Purpose
Establish overall system architecture and technical approach (foundational), then create detailed designs for specific increments (iterative). Translate requirements into implementable technical solutions.
...
### Outputs
- **Architecture Diagrams** - System components and interactions (see `design-architecture.md`)
...
### Handoff to Next Stage
Implementation stage receives: Architecture diagrams, detailed design for increment, API specs, data models, and test strategy.
...
### Purpose
Build working code that implements the detailed design for the current increment, following engineering best practices and professional standards.
...
### Handoff to Next Stage
Verification stage receives: Working code on branch/environment, unit test results, and implementation notes.
...
### Purpose
Validate that the increment works correctly through comprehensive testing and acceptance. Encompasses verification (built correctly) and validation (built the right thing).
...
### Outputs
- **Test Results** - All test types with pass/fail status (see `verification-report.md`)
...
### Handoff to Next Stage
Deployment stage receives: Verified code, test reports, UAT approval, and deployment checklist.
...
### Purpose
Release verified increments to production following controlled procedures, with monitoring and rollback capabilities.
...
### Handoff to Next Stage
Support stage receives: Deployed system, runbooks, monitoring dashboards, and baseline measurements for success criteria.
...
### Purpose
Monitor production systems, track success criteria, respond to incidents, fix bugs, and implement minor enhancements. Ensure system reliability and gather feedback for future improvements.
...
These stages repeat for each deliverable chunk of work. An "increment" might be called an epic, feature, sprint deliverable, or release increment depending on your methodology.
...
This stage runs continuously, supporting all deployed increments and feeding learnings back into future iterations.
...
This ensures goals aren't just documented and forgotten—they actively guide development and enable data-driven validation of success.
...
```
Initiation → Requirements → Design → Implementation → Verification → Deployment → Support
```
...
- `requirements-reference.md` - Requirements best practices
...
- `design-reference.md` - Design patterns and best practices
...
- `implementation-reference.md` - Coding standards and practices
...
- `verification-reference.md` - Testing strategies and approaches
...
- `deployment-reference.md` - Deployment best practices
...
- `support-reference.md` - Operations and maintenance guidance
...
Large organizations or complex projects may split Stage 3 (Design) into two stages:
...
These can be inserted as gates within stages rather than separate stages to maintain the 7-stage simplicity.

---

## Version History

- **1.0.0** (2025-02-08) - Initial 7-stage framework with measurement throughline
